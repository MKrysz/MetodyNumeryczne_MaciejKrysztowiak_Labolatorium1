...
def compare_plot(x1:np.ndarray,y1:np.ndarray,x2:np.ndarray,y2:np.ndarray,
    ...
    if x1.shape != y1.shape or x2.shape != y2.shape\
        or x1.shape != x2.shape or min(x1.shape)==0 or min(x2.shape)==0:
        return None
    fig, ax = plt.subplots()
...
def parallel_plot(x1:np.ndarray,y1:np.ndarray,x2:np.ndarray,y2:np.ndarray,
    ...
    if orientation == '-':
        orientation_value = 210
    elif orientation == '|':
        orientation_value = 120
    else:
        return None

    fig = plt.figure()
    plt.subplot(orientation_value + 1)
    plt.xlabel(x1label)
    plt.ylabel(y1label)
    plt.plot(x1, y1)

    plt.subplot(orientation_value + 2)
    plt.xlabel(x2label)
    plt.ylabel(y2label)
    plt.plot(x2, y2)
...
def log_plot(x:np.ndarray,y:np.ndarray,xlabel:np.ndarray,ylabel:str,title:str,log_axis:str):
    ...
    """
    if x.shape != y.shape or min(x.shape) == 0 or min(y.shape) == 0:
        return None
    fig = plt.figure()
    plt.plot(x, y)
    if log_axis.find('x') >= 0:
        plt.semilogx()
    if log_axis.find('y') >= 0:
        plt.semilogy()
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(title)
